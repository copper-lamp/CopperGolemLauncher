...
`installer.rs` 以数据库任务记录驱动 `downloading`、`extracting`、`installed`、`failed` 状态，并复用全局下载服务。`.appx` 走 ZIP 回退；`.msixvc` 先执行独立 XVC 头、哈希树、区域表、路径和原子提取校验，缺少 Store content key 时才进入兼容后端。完成后写入版本元数据并广播 `version.installed` 与模块事件，前端通过 i18n 键呈现状态。
